...
本文显示，预启动认证防止未经授权的攻击者访问加密硬盘内容。然而，启动前认证并不能阻止持有有效BitLocker PIN的恶意内部行为者获得设备本地管理访问权限，禁用杀毒软件或提取缓存凭证。这对共享系统尤为关键，因为它允许攻击者直接访问同一系统中其他用户的数据。

防止降级攻击的有效措施之一是更换TPM解封时检查的PCR。针对启动管理器漏洞 CVE-2024-38058，Microsoft 在测量启动进程中添加了PCR 4。该寄存器包含引导管理器代码的哈希值以及所有启动尝试。目前已有部分电脑的默认 PCR 值为 0，2，4，11。

用于降级攻击的漏洞启动管理器由 Microsoft Windows Production PCA 2011 签署。该证书有效期为15年，意味着其有效期为2026年6月。Microsoft UEFI CA 2011（用于签署第三方启动管理器）和 Microsoft Corporation KEK CA 2011（负责管理数据库和 DBX 内容）也将遭遇类似命运。 因此，Microsoft 于 2023 年注册了一套新的根证书;用于签署Windows启动管理器时，将使用新的Windows UEFI CA 2023。目前该证书授权机构的注册尚未自动完成，但可以通过手动应用补丁程序KB5025885完成。该补丁将新的CA添加到数据库，安装由2023年CA签署的启动管理器，并通过将2011年CA添加到DBX数据库中来撤销。
